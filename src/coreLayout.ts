 ...
 * Two GPU-mirrored core buffers only: `position` (vec3) and `lifecycle` (vec2 [age, lifetime]);
 ...
 *
 * Split into two buffers because a Three instanced attribute is at most a vec4 - a 5-float core
 * cannot be one attribute, whereas vec3 + vec2 are two clean varyings. Dependency-free (the editor's
...
export const FX_CORE_LIFECYCLE_STRIDE = 2;
